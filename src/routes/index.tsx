    ...
    <div>
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-28 md:py-36 lg:py-44 text-primary-foreground">
          <div className="max-w-4xl text-center mx-auto">
            ...
            <p className="text-lg md:text-xl lg:text-2xl max-w-3xl mx-auto opacity-90 leading-relaxed mb-10">Une plateforme d'apprentissage du Coran, du tajwid, des hadiths, du fiqh et de la langue arabe avec un accompagnement étape par étape.</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link to="/inscription" className="inline-flex items-center gap-2 px-8 py-4 rounded-full font-semibold text-primary hover:scale-[1.03] transition-transform" style={{ background: "var(--gradient-gold)", boxShadow: "var(--shadow-elegant)" }}>Commencer mon inscription <ArrowRight className="w-4 h-4" /></Link>
              <Link to="/cours" className="inline-flex items-center gap-2 px-8 py-4 rounded-full font-semibold border border-white/25 bg-white/5 hover:bg-white/10 transition-colors">Voir les formations</Link>
            </div>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-10 -mt-7 relative z-10">
        <div className="grid md:grid-cols-3 gap-4">
          {[["1", "Choisissez", "Votre formation et votre niveau"], ["2", "Inscrivez-vous", "Recevez votre code de suivi"], ["3", "Progressez", "Suivez votre parcours et vos séances"]].map(([n, title, text]) => <div key={n} className="rounded-2xl bg-card border border-border shadow-lg p-5 flex gap-4"><div className="w-10 h-10 rounded-full shrink-0 flex items-center justify-center font-bold text-primary" style={{ background: "var(--gradient-gold)" }}>{n}</div><div><p className="font-semibold text-primary">{title}</p><p className="text-sm text-muted-foreground mt-1">{text}</p></div></div>)}
        </div>
      </section>

      <section id="apropos" className="max-w-7xl mx-auto px-6 py-16 md:py-24">
        <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-center">
          <div><p className="text-xs uppercase tracking-[0.3em] text-accent mb-3">À propos</p><h2 className="text-4xl md:text-5xl text-primary mb-5">Une académie pensée pour apprendre avec sérénité</h2><p className="text-muted-foreground text-lg leading-relaxed mb-6">Nouroul Foua'ad propose une expérience simple : découvrir les programmes, s'inscrire, effectuer son paiement, suivre son dossier et avancer dans son apprentissage depuis un même espace.</p><div className="grid sm:grid-cols-2 gap-3">{["Programmes lisibles", "Suivi d'inscription", "Paiement par moyens locaux", "Assistance WhatsApp", "Espace étudiant", "Informations centralisées"].map((item) => <div key={item} className="flex gap-2 items-center text-sm text-primary"><CheckCircle2 className="w-4 h-4 text-accent" />{item}</div>)}</div></div>
          <div className="rounded-3xl border border-border bg-card p-7 md:p-9 shadow-xl"><div className="flex items-center gap-3 mb-6"><div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ background: "var(--gradient-gold)" }}><GraduationCap className="w-6 h-6 text-primary" /></div><div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Notre méthode</p><p className="text-xl text-primary font-semibold">Clarté · Régularité · Suivi</p></div></div><div className="space-y-4">{["Objectif pédagogique clair", "Séances et révisions régulières", "Échange avec l'équipe", "Progression visible dans l'espace étudiant"].map((x, i) => <div key={x} className="flex gap-4"><div className="w-7 h-7 rounded-full flex items-center justify-center bg-primary text-primary-foreground text-xs font-bold">{i + 1}</div><p className="text-sm text-muted-foreground pt-1">{x}</p></div>)}</div></div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">{subjects.map((s) => <article key={s.fr} className="group p-6 rounded-3xl bg-card border border-border hover:border-accent hover:-translate-y-1 transition-all shadow-sm hover:shadow-lg"><div className="text-3xl mb-4">{s.icon}</div><div className="font-arabic text-2xl text-accent mb-3">{s.ar}</div><h3 className="text-2xl text-primary mb-3">{s.fr}</h3><p className="text-sm text-muted-foreground leading-relaxed mb-5">{s.desc}</p><Link to={s.fr === "Langue arabe" ? "/arabe" : "/cours"} className="inline-flex items-center gap-2 text-sm font-semibold text-primary">En savoir plus <ArrowRight className="w-4 h-4" /></Link></article>)}</div>
      </section>

      <section className="max-w-7xl mx-auto px-6 py-16"><div className="text-center mb-12"><p className="text-xs uppercase tracking-[0.3em] text-accent mb-3">Pourquoi nous choisir</p><h2 className="text-4xl md:text-5xl text-primary">Une expérience simple et professionnelle</h2></div><div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">{values.map(({ icon: Icon, title, text }) => <article key={title} className="rounded-3xl bg-card border border-border p-6"><Icon className="w-6 h-6 text-accent mb-5" /><h3 className="text-xl text-primary mb-2">{title}</h3><p className="text-sm text-muted-foreground leading-relaxed">{text}</p></article>)}</div></section>
      ...
      <section className="max-w-7xl mx-auto px-6 py-16"><div className="grid lg:grid-cols-2 gap-8 items-stretch"><div className="rounded-3xl border border-border bg-card p-8 md:p-10"><p className="text-xs uppercase tracking-[0.3em] text-accent mb-3">Parole inspirante</p><p className="font-arabic text-4xl md:text-5xl text-primary leading-relaxed mb-5">اقْرَأْ بِاسْمِ رَبِّكَ الَّذِي خَلَقَ</p><p className="text-lg italic text-muted-foreground">« Lis, au nom de ton Seigneur qui a créé. » — Sourate Al-‘Alaq, 1</p></div><div className="rounded-3xl border border-border bg-card p-8 md:p-10"><p className="text-xs uppercase tracking-[0.3em] text-accent mb-3">Hadith</p><p className="font-arabic text-3xl text-primary leading-loose mb-5">خَيْرُكُمْ مَنْ تَعَلَّمَ الْقُرْآنَ وَعَلَّمَهُ</p><p className="text-muted-foreground">« Le meilleur d'entre vous est celui qui apprend le Coran et l'enseigne. »</p></div></div></section>

      <section id="faq" className="max-w-5xl mx-auto px-6 py-16"><div className="text-center mb-12"><p className="text-xs uppercase tracking-[0.3em] text-accent mb-3">FAQ</p><h2 className="text-4xl md:text-5xl text-primary">Questions fréquentes</h2></div><div className="space-y-3">{faqs.map(([q, a]) => <details key={q} className="group rounded-2xl border border-border bg-card px-6 py-5"><summary className="cursor-pointer list-none font-semibold text-primary flex items-center justify-between gap-4"><span>{q}</span><span className="text-accent text-xl group-open:rotate-45 transition-transform">+</span></summary><p className="mt-4 pt-4 border-t border-border text-sm leading-relaxed text-muted-foreground">{a}</p></details>)}</div></section>
