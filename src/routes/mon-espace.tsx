...
import { Award, BookOpen, CheckCircle2, ClipboardCheck, CreditCard, MessageCircle, ShieldCheck, UserRound } from "lucide-react";
      ...
      {data && (
        <div className="space-y-8">
          ...
          <section className="grid md:grid-cols-3 gap-4">
            {[[CreditCard, "Paiement", "/paiement", "Consulter les moyens de paiement"], [BookOpen, "Formations", "/cours", "Voir les programmes disponibles"], [MessageCircle, "Assistance", "https://wa.me/22788376133", "Contacter l'équipe sur WhatsApp"]].map(([Icon, title, href, text]) => { const I = Icon as typeof BookOpen; const external = String(href).startsWith("http"); const content = <div className="rounded-3xl bg-card border border-border p-6 hover:border-accent hover:-translate-y-0.5 transition-all"><I className="w-6 h-6 text-accent mb-4" /><p className="font-semibold text-primary">{title as string}</p><p className="text-sm text-muted-foreground mt-1">{text as string}</p></div>; return external ? <a key={title as string} href={href as string} target="_blank" rel="noopener noreferrer">{content}</a> : <Link key={title as string} to={href as "/paiement"}>{content}</Link>; })}
          </section>
        </div>
      )}
