...
export const getStudentDashboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    // Toutes les données de lecture du tableau de bord utilisent le client authentifié
    // injecté par le middleware. Le tableau de bord ne doit pas dépendre de la
    // Service Role Key pour s'afficher côté étudiant.
    ...
    const [coursesRes, enrollmentsRes, progressRes, quranRes, assessmentsRes, attendanceRes, notificationsRes] = await Promise.all([
      ...
      db.from("student_enrollments").select("id, status, enrolled_at, course_id, inscription_id, student_courses(id, slug, title, subject, description, level, schedule)").eq("user_id", context.userId).order("enrolled_at", { ascending: false }),
      db.from("student_progress").select("id, progress, last_activity_at, teacher_note, course_id, student_courses(title, subject)").eq("user_id", context.userId).order("updated_at", { ascending: false }),
      db.from("quran_progress").select("id, surah_number, surah_name, memorization_percent, revision_percent, teacher_note, updated_at").eq("user_id", context.userId).order("surah_number"),
      db.from("student_assessments").select("id, subject, title, score, max_score, assessed_at, teacher_note").eq("user_id", context.userId).order("assessed_at", { ascending: false }).limit(10),
      db.from("student_attendance").select("id, session_date, status, notes, course_id, student_courses(title)").eq("user_id", context.userId).order("session_date", { ascending: false }).limit(30),
      db.from("student_notifications").select("id, title, message, type, read_at, created_at").eq("user_id", context.userId).order("created_at", { ascending: false }).limit(12),
    ]);
...
export const updateStudentProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("student_profiles")
      .upsert({ user_id: context.userId, full_name: data.full_name, level: data.level || null }, { onConflict: "user_id" });
    if (error) throw new Error(error.message);
    return { ok: true };
  });
